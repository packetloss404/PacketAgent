...
- [ ] Close the inherited repo-wide Prettier baseline. `npm run format:check` currently flags 289 files; files authored in the R1 review slices are formatted.
- [ ] Reduce the inherited ESLint baseline of 50 frontend warnings to zero while preserving the current zero-error gate; backend warnings are closed.
