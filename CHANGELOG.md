...
- Added committed README captures of Builder app mode and canonical Worker
  operations, generated by the authenticated browser verifier.
- Added `npm run verify:docs` and made it part of the release gate. It checks
  all tracked Markdown, local links and screenshot targets, documented npm
  commands, historical labels, loop authority, and the conditional backlog
  boundary.
- Reconciled quick-start, self-hosting, security, PacketADE, and completed
  R1/R3-R6 evidence wording with the closed PA0/W1-W10/R1-R8 ledger.
