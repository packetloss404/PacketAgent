...
The following implemented subsystems are exercised by tests. Most are inherited and wired into `src/server.ts`; W1-W4 add the canonical Worker control plane, trigger-intake boundary, and bounded execution supervisor. Full checkpoint recovery and side-effect safety remain tracked separately in [the roadmap](dev/roadmap.md) and [backlog](BACKLOG.md).

- **Canonical Worker control plane and supervisor** (`src/workers/`). W1-W4 provide versioned records and runtime validators; immutable definitions, versions, and deployments; lifecycle commands and audit events; a durable activation envelope/inbox; encrypted expiring references for large or sensitive inputs; atomic admission of one version-pinned queued run and execution job; and a port-isolated plan-act-evaluate-checkpoint-decide supervisor. Manual, timezone-aware cron, opaque webhook, alert, and queue deliveries share this path across JSON, SQLite, and managed Postgres. Every run is bounded by elapsed time, iterations, provider cost, consecutive failures, retries, tool calls, cancellation, revocation, and a renewable fenced lease.
...
- **Honest about what is not built yet.** Trigger delivery now creates durable canonical queued Worker runs and `worker.run` executes them through the bounded supervisor. W5 still needs full immutable checkpoints, restart recovery, and external-effect receipts before Worker execution is crash-resumable and replay-safe. Other remaining limits include whole-tool rather than verb/resource-scoped grants, no default live SMTP transport, opt-in real sandbox validation, and legacy template artifacts that may still use browser-side sql.js.
...
7. Do not assume crash-resumable autonomous Workers exist yet; W1-W4 define the contract, persistence, lifecycle, durable trigger intake, and bounded execution, while W5-W8 build recovery, effect safety, policy, and controls.
...
The W4 validation ran **1,337 API tests** (1,336 passed, 1 skipped) plus **25 web tests** with no failures, covering the Worker contract, lifecycle, activation replay/races, bounded supervisor, tool-boundary permission denial, cancellation, fencing, optimistic revisions, JSON/SQLite/managed-Postgres parity, trigger adapters, routes, jobs, tools, and UI helpers.
...
- **Worker execution is bounded, but not crash-resumable yet.** W1-W4 provide versioned Worker records, lifecycle routes, a durable activation inbox, normalized trigger delivery, atomic canonical queued-run/job creation, and bounded plan-act-evaluate-checkpoint-decide execution across JSON, SQLite, and managed Postgres. W5 still owns full immutable checkpoints, restart recovery, and effect receipts; finer runtime policy and operator controls remain W6-W8 work.
...
PacketAgent is in its foundation transition from TaskLoom's app/agent workbench to the autonomous-worker runtime described at the top of this file. The inherited Builder and operate surfaces work; the rename and compatibility layer are committed on the current foundation branch. The canonical Worker model, lifecycle, W3 trigger intake, and W4 bounded-supervisor gates are complete: current trigger sources create deduplicated, version-pinned runs that execute within explicit time, iteration, cost, failure, tool-call, cancellation, revocation, and fenced-lease bounds.

The exact resume point is W5.1 in [BACKLOG.md](BACKLOG.md). New Codex projects should begin with [dev/CODEX-HANDOFF.md](dev/CODEX-HANDOFF.md), not the archived Phase 3 or legacy handoff documents.
