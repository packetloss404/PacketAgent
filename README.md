...
7. Canonical Workers are bounded and crash-resumable through W1-W5. W6 compiles and enforces deployment-scoped verb/resource grants, resolves opaque credentials only after approval, pins public network destinations, requires isolated Docker execution for Worker commands, atomically reserves rolling provider/action budgets, and closes direct-registry plus adversarial bypasses. Until W7-W8 pass, do not treat that boundary as proof that operator controls or consolidated evidence are complete.
...
The W7.4 validation ran **1,427 API tests** (1,426 passed, 1 skipped) plus **25 web tests** with no failures, covering the Worker contract, lifecycle, durable attention/approval/control/notification invariants, independently authorized inspect/run/deployment/approval routes, redacted operator projections, strict mutation bodies, no-store first-use approval nonces, workspace isolation, atomic pause/resume/stop/revoke races, approval scope/expiry and nonce non-persistence, approval-required checkpointing, exact one-time and run-scoped grant consumption, deadline escalation deduplication, pause/reject expiration, execution-boundary rechecks, transaction rollback, supervisor fencing, paused-job draining, graph integrity, workspace export, every production tool through executor and direct-registry paths, denial-before-credential/budget/effect/network ordering, linked and case-aliased host paths, hostile command arguments, atomic workspace/deployment rolling reservations, worst-case provider holds, billable-action ordering, idempotent settlement/release, lease-expiry reconciliation, credential isolation, A/AAAA and connected-address validation, redirect denial, Docker-only execution, capability compilation and narrowing, normalized operations, immediate policy enforcement, redacted allow/deny decisions, compiled-policy integrity, activation replay/races, bounded supervisor, immutable checkpoint chains, effect replay/reconciliation, recovery, quarantine, cancellation, fencing, optimistic revisions, JSON/SQLite/managed-Postgres parity, trigger adapters, routes, jobs, tools, and UI helpers.
...
- **Worker permission, budget, internal control, supervisor attention, and independent operator access are enforced, but the restart/evidence gates are not complete.** W1-W5 provide versioned Worker records, lifecycle routes, durable trigger intake, bounded execution, immutable full-state checkpoints, scheduler recovery, and mutation effect receipts across JSON, SQLite, and managed Postgres. W6 compiles version-bound verb/resource grants, rejects deployment broadening, normalizes and authorizes each operation, resolves only declared opaque credentials, pins public network destinations, denies redirects, requires isolated no-network Docker command execution, atomically reserves provider cost and externally billable actions across rolling workspace/deployment windows, and prevents direct registered-handler bypass. W7.1 persists bound approval/control records, nonce digests, and replay identities; W7.2 executes their atomic internal service; W7.3 creates and resumes durable approval attention with deadline enforcement; W7.4 exposes concise independently authorized operator routes. W7.5-W8 still own restart/kill closure, external notification transports, and consolidated evidence.
...
PacketAgent is in its foundation transition from TaskLoom's app/agent workbench to the autonomous-worker runtime described at the top of this file. The inherited Builder and operate surfaces work; the rename and compatibility layer are committed on the current foundation branch. The canonical Worker model, lifecycle, trigger intake, bounded-supervisor, W5 recovery/effect-safety, and W6 permission/budget gates are complete. W7.1 persists immutable run-bound attention, approval, control-command, and notification-delivery records in every backend, including durable consumption/replay fields. W7.2 applies those controls atomically with revision fencing, idempotent replay, durable rejection, exact-run resume, deployment-wide revoke, and digest-only approval nonces. W7.3 turns approval-required tool operations into checkpointed, schedulable attention and consumes only exact, unexpired grants at the execution boundary. W7.4 mounts an independent operator API with separately authorized run, deployment, and approval controls plus redacted run/attention inspection. Current trigger sources create deduplicated, version-pinned runs that execute within explicit bounds and recover from immutable snapshots without repeating a completed mutation or double-crediting an abandoned budget hold.

The exact resume point is W7.5 in [BACKLOG.md](BACKLOG.md). New Codex projects should begin with [dev/CODEX-HANDOFF.md](dev/CODEX-HANDOFF.md), not the archived Phase 3 or legacy handoff documents.
