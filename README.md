...
- **Honest about what is not built yet.** Trigger delivery creates durable canonical queued Worker runs, and `worker.run` executes them through a bounded, checkpointed, restart-recoverable supervisor with external-effect receipts. W6 still needs verb/resource-scoped runtime enforcement, credential-reference resolution, deny-by-default process/network boundaries, and rolling budgets. Other remaining limits include no default live SMTP transport, opt-in real sandbox validation, and legacy template artifacts that may still use browser-side sql.js.
...
7. Canonical Workers are bounded and crash-resumable through W1-W5. W6.1-W6.2 compile deployment-scoped verb/resource grants and enforce them again immediately before every production Worker tool handler. Until W6-W8 pass, do not treat that boundary as proof that credential resolution, SSRF/DNS controls, process isolation, rolling budgets, or operator controls are complete.
...
The W6.2 validation ran **1,361 API tests** (1,360 passed, 1 skipped) plus **25 web tests** with no failures, covering the Worker contract, lifecycle, capability compilation and narrowing, normalized operations, immediate policy enforcement, redacted allow/deny decisions, compiled-policy integrity, activation replay/races, bounded supervisor, immutable checkpoint chains, effect replay/reconciliation, expired-work recovery, quarantine, cancellation, fencing, optimistic revisions, JSON/SQLite/managed-Postgres parity, trigger adapters, routes, jobs, tools, and UI helpers.
...
- **Worker tool policy is enforced, but the wider permission and control gate is not complete.** W1-W5 provide versioned Worker records, lifecycle routes, durable trigger intake, bounded execution, immutable full-state checkpoints, scheduler recovery, and mutation effect receipts across JSON, SQLite, and managed Postgres. W6.1-W6.2 compile version-bound verb/resource grants, reject deployment broadening, normalize each production tool operation, and fail closed in `executeTool` before its handler. W6.3-W8 still own credential-reference resolution, network/process/filesystem hardening, rolling budgets, approvals, kill controls, and consolidated evidence.
...
- **Outbound tools have immediate policy scope, but their ambient security dependencies still need hardening.** `http_fetch`, `slack_post_webhook`, `github_api`, `email_send`, `sql_query`, and `shell_for_agent` expose normalized verb/resource operations, are checked against the pinned compiled policy in `executeTool`, and wrap allowed mutations in prepared/completed receipts. Approval tokens remain on the inherited agent path, email needs an injected SMTP adapter, and W6.3 must replace ambient credentials plus native network/process assumptions with Worker-scoped resolvers and hardened drivers.
...
PacketAgent is in its foundation transition from TaskLoom's app/agent workbench to the autonomous-worker runtime described at the top of this file. The inherited Builder and operate surfaces work; the rename and compatibility layer are committed on the current foundation branch. The canonical Worker model, lifecycle, trigger intake, bounded-supervisor, and W5 recovery/effect-safety gates are complete. W6.1-W6.2 compile requested capabilities into immutable deployment-scoped policies that can only narrow the version and authorize each normalized production tool operation immediately before its handler. Current trigger sources create deduplicated, version-pinned runs that execute within explicit bounds and recover from immutable snapshots without repeating a completed mutation.

The exact resume point is W6.3 in [BACKLOG.md](BACKLOG.md). New Codex projects should begin with [dev/CODEX-HANDOFF.md](dev/CODEX-HANDOFF.md), not the archived Phase 3 or legacy handoff documents.
