...
- **PacketBench** is where development work is planned, built, and supervised.
...
The target handoff is simple: build or plan work in PacketBench, choose **Deploy to PacketAgent** or **Keep running**, and receive progress, approval, budget, and completion events back through the Packet surfaces.
...
- [dev/packetbench-packetagent-handoff.md](dev/packetbench-packetagent-handoff.md) defines the current PacketBench identity and its additive WorkerPackage v1 compatibility rules.
- [dev/packetade-packetagent-handoff.md](dev/packetade-packetagent-handoff.md) retains the frozen pre-rename W9 contract and legacy identifiers.
...
7. Canonical Workers are bounded and crash-resumable through W1-W5. W6 compiles and enforces deployment-scoped verb/resource grants, resolves opaque credentials only after approval, pins public network destinations, requires isolated Docker execution for Worker commands, atomically reserves rolling provider/action budgets, and closes direct-registry plus adversarial bypasses. W7 adds independently authorized attention, approval, pause, resume, stop, and revoke controls whose restart and race gate passes. W8 adds the digest-bound evidence trail, deterministic rollups, retention, operations API, and accessible workbench. W9 completes the strict PacketBench package boundary with frozen PacketADE compatibility, receipt-bound deployment/control API, reconnectable event/cursor contract, and serialized disconnect/restart handoff gate. W10 adds the notification outbox, PacketChat delivery, PacketPhone controls, and a passing local remote-control certification gate; live interoperability stays conditional.
...
- **The canonical runtime and Packet-side handoff/control adapters pass their local gates; live product interoperability is not certified.** W1-W5 provide versioned Worker records, lifecycle routes, durable trigger intake, bounded execution, immutable full-state checkpoints, scheduler recovery, and mutation effect receipts across JSON, SQLite, and managed Postgres. W6 compiles version-bound verb/resource grants, rejects deployment broadening, normalizes and authorizes each operation, resolves only declared opaque credentials, pins public network destinations, denies redirects, requires isolated no-network Docker command execution, atomically reserves provider cost and externally billable actions across rolling workspace/deployment windows, and prevents direct registered-handler bypass. W7 persists and executes exact attention/control state, exposes concise independently authorized operator routes, and closes restart/callback/control/activation races without later work. W8 persists versioned evidence and artifact provenance, rebuilds deterministic cumulative rollups, applies bounded redaction/retention with deletion evidence, and exposes the consolidated server-side operations read model, cursor APIs, resumable SSE, and accessible canonical Worker workbench. W9 defines strict digest-bound WorkerPackage v1 bytes, Ed25519 DSSE verification against a workspace-scoped public signing-key registry (`packet-product-signing-key add|revoke|list`), authenticated workspace-bound Packet-product actors, durable local-policy receipts, the receipt-bound deployment/control API, reconnectable events with explicit acknowledgements, and a passing disconnect/restart contract gate. It accepts current PacketBench source identity and frozen PacketADE compatibility identity. W10 adds the durable notification outbox, encrypted pinned-network PacketChat delivery with exact-binding read-only callbacks, encrypted HTTPS-only PacketPhone delivery with role-bounded durable single-use W7 controls, and a local certification matrix covering race orderings, restart, replay, credential rotation, and audited dead-letter redrive. PacketChat has not implemented its receiving half; PacketPhone has not been configured for a live probe.
...
Packet-product-only `WorkerPackage v1` source boundary. That boundary accepts
current PacketBench and frozen PacketADE identity pairs only. A strict signed
