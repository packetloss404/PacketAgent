...
7. Canonical Workers are bounded and crash-resumable through W1-W5, and W6.1 compiles deployment-scoped verb/resource grants. Until W6-W8 pass, treat that compiled policy as an upper bound rather than proof that every credential, network, filesystem, shell, rolling-budget, and operator-control boundary is enforced.
...
- **Worker execution is bounded and crash-resumable, but compiled policy is not enforced everywhere yet.** W1-W5 provide versioned Worker records, lifecycle routes, durable trigger intake, bounded execution, immutable full-state checkpoints, scheduler recovery, and mutation effect receipts across JSON, SQLite, and managed Postgres. W6.1 compiles version-bound verb/resource grants and rejects deployment broadening. W6.2-W8 still own mandatory tool-boundary enforcement, credential and process/network hardening, rolling budgets, approvals, kill controls, and consolidated evidence.
...
- **Outbound tools have durable effects but not yet Worker-grade policy scope.** `http_fetch`, `slack_post_webhook`, `github_api`, `email_send`, `sql_query`, and `shell_for_agent` are registered, explicitly effect-classified, and wrapped in prepared/completed receipts for Worker execution. Approval tokens are currently whole-tool scoped, email needs an injected SMTP adapter, and W6 must close credential, network, filesystem, shell, and direct-registry bypasses.
