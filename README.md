...
7. Canonical Workers are bounded and crash-resumable through W1-W5. W6.1-W6.4 compile and enforce deployment-scoped verb/resource grants, resolve opaque credentials only after approval, pin public network destinations, require isolated Docker execution for Worker commands, and atomically reserve rolling provider/action budgets. Until W6-W8 pass, do not treat that boundary as proof that the final bypass matrix or operator controls are complete.
...
- **Worker tool policy and rolling budgets are enforced, but the wider permission and control gate is not complete.** W1-W5 provide versioned Worker records, lifecycle routes, durable trigger intake, bounded execution, immutable full-state checkpoints, scheduler recovery, and mutation effect receipts across JSON, SQLite, and managed Postgres. W6.1-W6.4 compile version-bound verb/resource grants, reject deployment broadening, normalize and authorize each operation, resolve only declared opaque credentials, pin public network destinations, deny redirects, require isolated no-network Docker command execution, and atomically reserve provider cost and externally billable actions across rolling workspace/deployment windows. W6.5-W8 still own the final bypass matrix, approvals, kill controls, and consolidated evidence.
...
PacketAgent is in its foundation transition from TaskLoom's app/agent workbench to the autonomous-worker runtime described at the top of this file. The inherited Builder and operate surfaces work; the rename and compatibility layer are committed on the current foundation branch. The canonical Worker model, lifecycle, trigger intake, bounded-supervisor, and W5 recovery/effect-safety gates are complete. W6.1-W6.4 compile requested capabilities into immutable deployment-scoped policies that can only narrow the version, authorize each normalized production tool operation immediately before its handler, resolve only version-declared opaque credentials, pin public network destinations, isolate Worker commands in Docker, and reserve durable rolling provider/action capacity before external execution. Current trigger sources create deduplicated, version-pinned runs that execute within explicit bounds and recover from immutable snapshots without repeating a completed mutation or double-crediting an abandoned budget hold.

The exact resume point is W6.5 in [BACKLOG.md](BACKLOG.md). New Codex projects should begin with [dev/CODEX-HANDOFF.md](dev/CODEX-HANDOFF.md), not the archived Phase 3 or legacy handoff documents.
