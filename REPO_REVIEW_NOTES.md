# PacketAgent Repo Review - Running Notes

> **Status: archived review notes.** Facts and counts below were captured before
> the PacketAgent foundation transition and are retained only as historical
> evidence. Current state lives in
> [`dev/CODEX-HANDOFF.md`](dev/CODEX-HANDOFF.md).

## High-level facts (verified)

- Project: `packetagent` v0.1.0, MIT, repo github.com/packetloss404/packetagent
...
- `data/`: generated-apps, published-apps, packetagent.json (495KB local data, gitignored)

## RED FLAGS (initial)

- HUGE files (lines): operations-status.ts 5069, packetagent-store.ts 4941, app-routes.ts 4176, packetagent-services.ts 3771, release-readiness.ts 3431, app-builder-service.ts 3255, release-evidence.ts 2976, db/cli.ts 2515, operations-health.ts 2238
...
- Top-level log files present (packetagent-api.err.log 204KB etc.) - gitignored (not tracked) but left in working tree
