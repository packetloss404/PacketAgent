...
| Jobs have multiple writers and incomplete workspace scoping | Open audit | R1 must re-run the repository/job-service race and tenant-isolation scenarios before assigning a final status. |
...
| Startup truth, rate-limit identity, scheduler rejection, artifact scope, response headers/CSP | Open audit | Inspect current server and route tests, reproduce only still-applicable findings, and close them in the next R1 backend/security slice. |
