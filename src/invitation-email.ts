...
import {
  ...
  type InvitationEmailDeliveryMode,
...
export const PACKETAGENT_INVITATION_EMAIL_WEBHOOK_URL_ENV =
  "PACKETAGENT_INVITATION_EMAIL_WEBHOOK_URL";
export const PACKETAGENT_INVITATION_EMAIL_WEBHOOK_SECRET_ENV =
  "PACKETAGENT_INVITATION_EMAIL_WEBHOOK_SECRET";
export const PACKETAGENT_INVITATION_EMAIL_WEBHOOK_SECRET_HEADER_ENV =
  "PACKETAGENT_INVITATION_EMAIL_WEBHOOK_SECRET_HEADER";
export const PACKETAGENT_INVITATION_EMAIL_WEBHOOK_TIMEOUT_MS_ENV =
  "PACKETAGENT_INVITATION_EMAIL_WEBHOOK_TIMEOUT_MS";
export const PACKETAGENT_INVITATION_EMAIL_RETRY_MAX_ATTEMPTS_ENV =
  "PACKETAGENT_INVITATION_EMAIL_RETRY_MAX_ATTEMPTS";
...
export function invitationEmailSubject(workspaceName?: string): string {
  ...
  return trimmedName
    ? `You're invited to ${trimmedName} on PacketAgent`
    : "You're invited to PacketAgent";
}

export function resolveInvitationEmailMode(
  value = process.env[PACKETAGENT_INVITATION_EMAIL_MODE_ENV],
...
export function resolveInvitationEmailWebhookConfig(
  env = process.env,
): InvitationEmailWebhookConfig {
  return {
    provider:
      env[PACKETAGENT_INVITATION_EMAIL_PROVIDER_ENV]?.trim() || WEBHOOK_INVITATION_EMAIL_PROVIDER,
    ...
    secretHeader:
      env[PACKETAGENT_INVITATION_EMAIL_WEBHOOK_SECRET_HEADER_ENV]?.trim() ||
      DEFAULT_INVITATION_EMAIL_WEBHOOK_SECRET_HEADER,
    timeoutMs: resolveInvitationEmailWebhookTimeoutMs(
      env[PACKETAGENT_INVITATION_EMAIL_WEBHOOK_TIMEOUT_MS_ENV],
    ),
  };
}
...
export function resolveInvitationEmailRetryMaxAttempts(env = process.env): number {
  const maxAttempts = Number.parseInt(
    env[PACKETAGENT_INVITATION_EMAIL_RETRY_MAX_ATTEMPTS_ENV] ?? "",
    10,
  );
  return Number.isInteger(maxAttempts) && maxAttempts > 0
    ? maxAttempts
    : DEFAULT_INVITATION_EMAIL_RETRY_MAX_ATTEMPTS;
}
...
export function recordLocalInvitationEmailDelivery(
  ...
  return createInvitationEmailDelivery(
    data,
    {
      id: input.deliveryId,
