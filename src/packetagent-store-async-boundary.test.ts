...
test("JSON async load and mutate reuse the current store behavior", async () => {
  await withStoreEnv({ PACKETAGENT_STORE: "json" }, async () => {
    ...
    clearStoreCacheForTests();
    assert.equal(
      (await loadStoreAsync()).requirements.some((entry) => entry.id === requirementId),
      true,
    );
    assert.equal(
      (await listRequirementsForWorkspaceIndexedAsync("alpha")).some(
...
test("managed and Postgres modes require a managed database URL for async wrappers", async () => {
  for (const storeMode of ["managed", "postgres", "postgresql"]) {
    await withStoreEnv({ PACKETAGENT_STORE: storeMode }, async () => {
      await assert.rejects(loadStoreAsync(), (error) => {
...
test("synchronous JSON and SQLite APIs still return direct values", async () => {
  await withStoreEnv({ PACKETAGENT_STORE: "json" }, () => {
    ...
    assert.equal(
      loadStore().requirements.some((entry) => entry.id === "req_sync_json_boundary"),
      true,
    );
  });
  ...
  try {
    await withStoreEnv(
      { PACKETAGENT_STORE: "sqlite", PACKETAGENT_DB_PATH: join(tempDir, "packetagent.sqlite") },
      () => {
        resetStoreForTests();
        const result = mutateStore((data) => {
          upsertRequirement(
        ...
        clearStoreCacheForTests();
        assert.equal(
          loadStore().requirements.some((entry) => entry.id === "req_sync_sqlite_boundary"),
          true,
        );
      },
    );
