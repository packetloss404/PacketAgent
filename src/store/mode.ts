...
const MANAGED_STORE_MODES = new Set([
  "managed",
  "managed-db",
  "managed-database",
  "postgres",
...
export class ManagedDatabaseStoreBoundaryError extends Error {
  ...
  constructor(resolution: ResolvedPacketAgentStoreMode) {
    ...
    super(
      hints
        ? `${MANAGED_DATABASE_SYNC_ADAPTER_GAP_MESSAGE} Unsupported managed database hint(s): ${hints}.`
...
export function resolvePacketAgentStoreMode(
  env: NodeJS.ProcessEnv = process.env,
): ResolvedPacketAgentStoreMode {
  const requestedStore = cleanStoreEnvValue(env.PACKETAGENT_STORE).toLowerCase();
  const managedDatabaseUrlKeys = MANAGED_DATABASE_URL_ENV_KEYS.filter(
    (key) => cleanStoreEnvValue(env[key]).length > 0,
  );
  const mode: PacketAgentStoreMode =
    requestedStore === "sqlite"
      ? "sqlite"
      : requestedStore === "json" || requestedStore === ""
        ? managedDatabaseUrlKeys.length > 0
          ? "managed"
          : "json"
        : MANAGED_STORE_MODES.has(requestedStore)
          ? requestedStore === "postgres" || requestedStore === "postgresql"
            ? "postgres"
            : "managed"
          : "json";
