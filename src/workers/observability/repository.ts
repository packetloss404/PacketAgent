  ...
  return {
    async listEvents(workspaceId, options = {}) {
      ...
      return data.workerEvents
        ...
        .map((event) => redactWorkerEventForRead(clone(event), secrets));
    },
    async listEvidence(workspaceId, options = {}) {
      const data = await loadStore();
      const secrets = await knownSecretValues(workspaceId);
      validateWorkerPersistence(data);
      return data.workerEvidenceEntries
        .filter(
          (entry) =>
        ...
        .sort((left, right) => left.sequence - right.sequence)
        .slice(0, limit(options.limit))
        .map((entry) => redactWorkerEvidenceForRead(clone(entry), secrets));
    },
    async listArtifactManifests(workspaceId, options = {}) {
      const data = await loadStore();
      const secrets = await knownSecretValues(workspaceId);
      validateWorkerPersistence(data);
      return data.workerArtifactManifests
        .filter(
          (manifest) =>
        ...
        .slice(0, limit(options.limit))
        .map((manifest) => redactWorkerArtifactManifestForRead(clone(manifest), secrets));
    },
  };
}
