  ...
  return (
    <div style={{ padding: "32px 28px", maxWidth: 880 }}>
      ...
      <p style={{ fontSize: 14.5, color: "var(--silver-300)", marginTop: 6, marginBottom: 22 }}>
        PacketAgent flips a workspace to <span style={{ color: "var(--green)" }}>activated</span>{" "}
        after the team publishes an app and configures alerts. You're {pct}% of the way there.
      </p>
      ...
      {detail.error && (
        <div className="card" style={{ padding: 22, color: "var(--danger)" }}>
          {detail.error}
        </div>
      )}
        ...
        <div className="card" style={{ padding: 22, marginBottom: 18 }}>
          ...
          {items.map((s, i) => (
            ...
            >
              ...
              >
                {s.completed ? (
                  <I.check size={13} stroke="#0E1A02" strokeWidth={3} />
                ) : (
                  <span className="mono" style={{ fontSize: 11, color: "var(--silver-400)" }}>
                    {i + 1}
                  </span>
                )}
              </div>
              <div style={{ flex: 1 }}>
                <div
                  style={{
                    fontSize: 14,
                    color: s.completed ? "var(--silver-300)" : "var(--silver-50)",
      ...
      <div className="card" style={{ padding: 18 }}>
        ...
        <p style={{ fontSize: 13.5, lineHeight: 1.65, color: "var(--silver-200)" }}>
          ...
          <span className="mono" style={{ color: "var(--green)" }}>
            onboarding → activating → activated
          </span>
          . The signals are real — agent runs, releases, alert routes — not vanity steps. We use
          them to decide when to surface advanced features (share tokens, audit log, op runbooks).
        </p>
      </div>
    </div>
  );
